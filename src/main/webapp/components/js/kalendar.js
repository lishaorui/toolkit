			...
			for(var n=0;n<7;n++){//列
				if((7*m+n) < codeFirst || (7*m+n) >= (codeFirst+monthDays[codeMonth])){//某月日历中不存在的日期
					...
					if((7*m+n+1-codeFirst == codeDay)&&(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6))){//当天是周末
						kalendar_html += "<li class='todayWeekend'>"+"<input type='checkbox' name='workday' value='"+(7*m+n+1-codeFirst)+"'/>"+(7*m+n+1-codeFirst)+"</li>";
					}else if(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6)){//仅是周末
						kalendar_html += "<li class='weekend'>"+"<input type='checkbox' name='workday' value='"+(7*m+n+1-codeFirst)+"'/>"+(7*m+n+1-codeFirst)+"</li>";
					}else if(7*m+n+1-codeFirst == codeDay){//仅是当天
						kalendar_html += "<li class='today'>"+"<input type='checkbox' name='workday' value='"+(7*m+n+1-codeFirst)+"'/>"+(7*m+n+1-codeFirst)+"</li>";
					}else{//其他日期
						kalendar_html += "<li>"+"<input type='checkbox' name='workday' value='"+(7*m+n+1-codeFirst)+"'/>"+(7*m+n+1-codeFirst)+"</li>";
					}
				}
			}
			kalendar_html += "</ul>\n";
		...
		*/
		...
		for(var i=0;i<7*newLine;i++){
			if(i < dateFirstDay || i> (dateTotalDays+dateFirstDay-1)){//日历中 当月不存在的日期
				...
				dateObj.eq(i).html("<input name='workday' type='checkbox' value='"+(i+1-dateFirstDay)+"'/>"+(i+1-dateFirstDay));
			}
		}
	}
